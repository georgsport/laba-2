...
void printMedianGrade();
double getMedianFromEvenVector(std::vector<int> input);
void printModes();
void shakerSort(std::vector<int>& input);
...
void analyseGrades() { // Функция анализа и вывода результатов (по факту сокращение для читаемости кода)
	printAverageGrade();   // Нахождение среднего
	printMedianGrade();	   // Нахождение медианы
	printModes();       // Нахождение моды
}

void printAverageGrade() { // Функция для нахождения среднего
...
void printMedianGrade() { // Функция нахождения медианы
	std::vector<int> sortedGrades = grades; // Вектор для сортировки оценок
	shakerSort(sortedGrades);				// Сортируем оценки
...
void printModes() { // Функция нахождения моды
